 ...
 * 
 */
@BaseBuyAnnotation(lotteryId = "320", salesType = "218", childId = "1")
public class JCPL320 extends JcBaseFragment {
	...
	private int chid;
	private boolean issp, isname; // sp 值 和 name 筛选标志
				...
				sDialog.addListener(gameNameMap, new OnClickListener() {
					...
					public void onClick(View arg0) {
						if (sDialog.getSelectListName().size() > 0) {
							...
							selectMessBean();
							if (gList.size() > 0) {
								mjcAdapter = new JCPL220ListAdapter(mActivity, gList, home_expandable_Lv);
			...
			if (list.size() > 0) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if ((!StringUtils.isBlank(m.getA()) && !StringUtils.isBlank(m.getB())) && SP > 0) {// SP值筛选
						if ((m.getA().contains(",") && m.getB().contains(",")) && (m.getA().split(",").length > 0 && m.getB().split(",").length > 0)) {
							// 1.5以下
							if ((1.5 == SP) && (Float.parseFloat(m.getA().split(",")[0]) > SP) && (Float.parseFloat(m.getB().split(",")[0]) > SP)) {
								issp = true;
							}
							// 2.0以上
							if ((2.0 == SP) && (Float.parseFloat(m.getA().split(",")[0]) < SP) && (Float.parseFloat(m.getB().split(",")[0]) < SP)) {
								issp = true;
							}
						} else {
	...
	protected View customContent_saleType() {
		...
		View v = View.inflate(mActivity, R.layout.f_zq_pop_titile_rb, null);
		v.findViewById(R.id.arrange3_rbtn2).setVisibility(View.GONE);
		v.findViewById(R.id.ll_zq_layout2).setVisibility(View.GONE);//隐藏足彩的一些东西
		return v;
	}
	...
	protected void onSaleTypeChanged(String tag) {
		...
		System.out.println("220------------s--> " + s + " c--> " + c);
		...
		case 218:// 胜平负 JC_D218Adapter
			...
			// setTitleNav(Integer.parseInt(lotteryId),
			// "胜平负",R.drawable.title_nav_back, R.drawable.title_nav_menu);
			// msg.put("I", "guding");
			// break;
			// default://单关
			// dg_btn.setChecked(true);
			setTitleNav(Integer.parseInt(lotteryId), "胜平负", R.drawable.title_nav_back, R.drawable.title_nav_menu);
			msg.put("I", "fudong");
			// break;
			// }
		...
		case 210:// 让球胜平负 JC_D210Adapter
			...
			// cg_btn.setChecked(true);
			// setTitleNav(Integer.parseInt(lotteryId),
			// "让球胜平负",R.drawable.title_nav_back, R.drawable.title_nav_menu);
			// msg.put("I", "guding");
			// break;
			// default://单关
			// dg_btn.setChecked(true);
			setTitleNav(Integer.parseInt(lotteryId), "让球胜平负", R.drawable.title_nav_back, R.drawable.title_nav_menu);
			msg.put("I", "fudong");
			// break;
			// }
		...
		// msg.put("A", Integer.parseInt(salesType));// salesType 209
		// msg.put("I", "guding");
		// msg.put("F", "0");
		// break;
		// //// 总进球
...
//			}
//		} else {
			mjcAdapter = new JCPL220ListAdapter(mActivity, groupList, home_expandable_Lv);
//		}
	...
	class JCPL220ListAdapter extends JCListAdapter {
		Map<String, List<String>> tmap = new HashMap<String, List<String>>();

		public JCPL220ListAdapter(Context context, List<List<MessageBean>> groupAllArray, JCListView listView) {
		...
		public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
			...
			case 209:
				...
				if (!StringUtils.isBlank(t.getB().toString()) && t.getB().toString().contains(",")) {
					...
					if (tempStr != null && tempStr.length == 3) {
						if (!StringUtils.isBlank(tempStr[0])) {// A 胜SP值
							cholder.cb3.setText(Html.fromHtml(getPKName("主胜", tempStr[0])));
							cholder.cb3.setTag("218^3_" + tempStr[0]);
						}
						if (!StringUtils.isBlank(tempStr[1])) {// B 平SP值
							cholder.cb1.setText(Html.fromHtml(getPKName("平", tempStr[1])));
							cholder.cb1.setTag("218^1_" + tempStr[1]);
						}
						if (!StringUtils.isBlank(tempStr[2])) {// C 负SP值
							cholder.cb0.setText(Html.fromHtml(getPKName("客胜", tempStr[2])));
				...
				if (!StringUtils.isBlank(t.getA().toString()) && t.getA().toString().contains(",")) {
					...
					if (tempStr != null && tempStr.length == 3) {
						if (!StringUtils.isBlank(tempStr[0])) {// A 胜SP值
							cholder.r_ch3.setText(Html.fromHtml(getPKName("主胜", tempStr[0])));
							cholder.r_ch3.setTag("210^3_" + tempStr[0]);
						}
						if (!StringUtils.isBlank(tempStr[1])) {// B 平SP值
							cholder.r_ch1.setText(Html.fromHtml(getPKName("平", tempStr[1])));
							cholder.r_ch1.setTag("210^1_" + tempStr[1]);
						}
						if (!StringUtils.isBlank(tempStr[2])) {// C 负SP值
							cholder.r_ch0.setText(Html.fromHtml(getPKName("客胜", tempStr[2])));
			...
			case 218:
				...
				if (!StringUtils.isBlank(a)) {// A 胜SP值
					cholder.cb3.setText(Html.fromHtml(getPKName(false, k, "主胜", a)));
					cholder.cb3.setTag("3_" + a);
				}
				if (!StringUtils.isBlank(b)) {// B 平SP值
					cholder.cb1.setText(Html.fromHtml(getPKName(false, "VS", "平", b)));
					cholder.cb1.setTag("1_" + b);
				}
				if (!StringUtils.isBlank(c)) {// C 负SP值
					cholder.cb0.setText(Html.fromHtml(getPKName(false, m, "客胜", c)));
			...
			case 210:
				...
				if (!StringUtils.isBlank(a)) {// A 胜SP值
					cholder.r_ch3.setText(Html.fromHtml(getPKName(false, k, "主胜", a)));
					cholder.r_ch3.setTag("3_" + a);
				}
				if (!StringUtils.isBlank(b)) {// B 平SP值
					cholder.r_ch1.setText(Html.fromHtml(getPKName(false, "VS", "平", b)));
					cholder.r_ch1.setTag("1_" + b);
				}
				if (!StringUtils.isBlank(c)) {// C 负SP值
					cholder.r_ch0.setText(Html.fromHtml(getPKName(false, m, "客胜", c)));
			...
			if (itemMB.getC().length() > 6) {// 20141122为了字符串截取安全
				key = itemMB.getC().subSequence(itemMB.getC().length() - 6, itemMB.getC().length()) + "-" + itemMB.getG();
			}
			// 组栏效果(默认)
			...
			cholder.item_chgp.setOnTextViewGroupListener(new OnTextViewGroup() {
				...
				public void setOnTextViewGroup(View v, List<String> tagList, List<String> textList) {
					...
					if (15 > ticket.getSelectMessageBean().size()) {// 客户端取消选票控制转由后台控制最多选票数
						...
						System.out.println("220---->" + ticket.getLotteryTag().toString());
						basebuy_tv_notice.setText(Html.fromHtml(ticket.getNotice()));
	...
	public static String getPKName(boolean isChecked, String pk, String spf, String sp) {
		...
		if (isChecked) {
			// <string name="jc_itemui_false"><![CDATA[NAME<br><small>PK
			// SP</small></br>]]></string>
	...
	public static String getPKName(String name, String sp) {
		...
		// if(isChecked){
		// <string name="jc_209item_true"><![CDATA[NAME
		// &#160;<small>SP</small>]]></string>
		s = StringUtils.replaceEach(MyApp.res.getString(R.string.jc_209item_true), new String[] { "NAME", "SP" }, new String[] { name, sp });
		// }else{
		// // <string name="jc_209item_false"><![CDATA[NAME<font
		// color="#606060">&#160;<small>SP</small></font>]]></string>
		// s=
		// StringUtils.replaceEach(MyApp.res.getString(R.string.jc_209item_true),
		// new String[]{"NAME","SP"}, new String[]{ name ,sp});
		// }
		return s;
	}
	...
	public static void setTagTextViewmsg(boolean isAddText, TextView tv, String L) {
		...
		if ("0".equals(L)) {// 0：表示不让 @color/btn_gray_u
			...
			tv.setBackgroundDrawable(MyApp.res.getDrawable(R.drawable.jc_r_gray));// 灰色
		} else if (L.contains("-")) {// -n：表示主让客 @color/red
			// tv.setBackgroundColor(MyApp.res.getColor(R.color.r_green));
			tv.setBackgroundDrawable(MyApp.res.getDrawable(R.drawable.jc_r_green));// 红色
	...
	protected void ok() {
		...
		if (ticket.isTicketAvailable()) {
			...
			System.out.println("321---childId---------->" + childId);
