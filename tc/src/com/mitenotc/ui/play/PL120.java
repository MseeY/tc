...
import com.mitenotc.ui.ui_utils.MyWM;
import com.mitenotc.ui.ui_utils.MyWM.ChangeyilouShowView;
...
@BaseBuyAnnotation(lotteryId = "120",salesType = "0",childId="1")
public class PL120 extends BaseBuyFragment implements OnCheckedChangeListener ,RefreshYiLou{
//	特殊Ticket
...
//	private MyWM wm;
    ...
    private void registerBroadcastReceiver(){  
         if(mReceiver==null){
        	 mReceiver =new RefreshYiLouReceiver("120"); 
        	 mReceiver.setmRefreshYiLou(this);
        	 IntentFilter mFilter=new IntentFilter();
        	 mFilter.addAction("com.mitenotc.ui.play.on_lottery_"+120+"_start_loading");
        	 mFilter.addAction("com.mitenotc.ui.play.on_lottery_"+120+"_stop_loading");
        	 mFilter.addAction("com.mitenotc.ui.play.on_lottery_"+120+"_awardinfo_received");
...
//			MyApp.saveProbeMsg("120-福彩快三","漏->号(当前选号--有遗漏)",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())
//		  			,"",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()),"");
		}else{
			itemChangeaddOmit(false);
//			MyApp.saveProbeMsg("120-福彩快三","漏<-号(当前选号--无遗漏)",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())
//		  			,"",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()),"");
		}
...
//	     		   R.color.fast3_text_color, R.color.bottom_nav_text_checked, R.drawable.fast3_choose_false);
		...
		containers[1].setActionUpListener(new ActionUpListener() {
			...
			public void onActionUp() {
				...
				basebuy_tv_notice.setText(Html.fromHtml(ticket.getNotice()));
				shakeListener.vibrate();
				changeBtnbgAndTextColors(ticket.getLotteryNums().get(1).size());
			}
		});
...
//		    		   R.color.fast3_text_color, R.color.bottom_nav_text_checked, R.drawable.fast3_choose_false);//复选
			...
			containers[2].setActionUpListener(new ActionUpListener() {
				...
				public void onActionUp() {
					...
					basebuy_tv_notice.setText(Html.fromHtml(ticket.getNotice()));
					shakeListener.vibrate();
					changeBtnbgAndTextColors(ticket.getLotteryNums().get(2).size());
