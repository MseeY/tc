...
@BaseBuyAnnotation(lotteryId = "122",salesType = "0",childId="1")
public class PL122 extends BaseBuyFragment implements OnCheckedChangeListener  ,RefreshYiLou{
//	特殊Ticket
...
//	private MyWM wm;
    ...
    private void registerBroadcastReceiver(){  
         if(mReceiver==null){
        	 mReceiver =new RefreshYiLouReceiver("122"); 
        	 mReceiver.setmRefreshYiLou(this);
        	 IntentFilter mFilter=new IntentFilter();
        	 mFilter.addAction("com.mitenotc.ui.play.on_lottery_"+122+"_start_loading");
        	 mFilter.addAction("com.mitenotc.ui.play.on_lottery_"+122+"_stop_loading");
        	 mFilter.addAction("com.mitenotc.ui.play.on_lottery_"+122+"_awardinfo_received");
...
//			MyApp.saveProbeMsg("122-福彩快三","漏->号(当前选号--有遗漏)",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())
//		  			,"",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()),"");
		}else{
			itemChangeaddOmit(false);
//			MyApp.saveProbeMsg("122-福彩快三","漏<-号(当前选号--无遗漏)",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())
//		  			,"",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()),"");
		}
...
//			 sampleTStr=MyApp.res.getStringArray(R.array.fast3_slh_tbonus_text);
		    ...
		    containers[0].setActionUpListener(new ActionUpListener() {
				...
				public void onActionUp() {
					...
					changeBtnbgAndTextColors(ticket.getLotteryNums().get(0).size()+
			                 ticket.getLotteryNums().get(1).size());
				}
			});
		    containers[1].setActionUpListener(new ActionUpListener() {
...
//		    		   R.color.fast3_text_color, R.color.bottom_nav_text_checked, R.drawable.fast3_choose_false);//复选
			...
			containers[0].setActionUpListener(new ActionUpListener() {
				...
				public void onActionUp() {
					...
					changeBtnbgAndTextColors(ticket.getLotteryNums().get(0).size()+
							                 ticket.getLotteryNums().get(1).size()+
							                 ticket.getLotteryNums().get(2).size());
				}
			});
			containers[1].setActionUpListener(new ActionUpListener() {
